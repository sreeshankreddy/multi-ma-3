...
try:
    ...
    st.error(f"⚠️ Failed to import required modules: {str(e)}")
...
def initialize_session_state():
    ...
    if 'analyzer' not in st.session_state:
        try:
            ...
            st.session_state.init_error = f"DocumentAnalyzer initialization failed: {str(e)}"
    ...
    if 'db' not in st.session_state:
        try:
            ...
            st.session_state.db_error = f"Database initialization failed: {str(e)}"
...
def show_login_page():
    ...
    with col2:
        ...
        if st.session_state.db is None:
            st.error("Database connection failed. Please try restarting the application.")
            return
...
def show_document_upload():
    """Display document upload interface with error handling."""
    st.header("📤 Upload Document")

    if st.session_state.analyzer is None:
        st.error("❌ Document analyzer not available. The app will work with limited functionality.")
        st.info("Supported formats: PDF, JPG, PNG, DOCX, TXT")
        ...
        try:
            ...
            with st.status("Processing...", expanded=True) as status:
                ...
                if st.button("🔍 Analyze Document", use_container_width=True):
                    if st.session_state.analyzer is None:
                        st.error("Analyzer not initialized. Cannot process document.")
                        return
                    ...
                    try:
                        ...
                        if success:
                            ...
                            # Save to database
                            try:
                                if st.session_state.db and st.session_state.current_user_id:
                                    ...
                                    st.session_state.db.save_analysis_result(
                                        ...
                                        text[:5000],
                                        str(analyzer.tables) if analyzer.tables else "[]"
                                    )
                            except Exception as db_e:
                                st.warning(f"Could not save to database: {str(db_e)}")
...
def show_document_chat():
    """Display document Q&A interface with error handling."""
                ...
                try:
                    ...
                    with st.spinner("Thinking..."):
                        ...
                        if 'error' not in result:
                            ...
                            confidence = result.get('confidence', 0) if isinstance(result.get('confidence'), (int, float)) else 0
                            ...
                            # Save to database
                            try:
                                if st.session_state.db and st.session_state.current_document_id:
                                ...
                                st.warning(f"Could not save chat: {str(db_e)}")
...
def show_document_history():
    """Display user's document upload history with error handling."""
...
def show_report_generation():
    """Display report generation interface with error handling."""
    ...
    if st.button("Generate Reports", use_container_width=True):
        try:
            ...
            with st.spinner("Generating reports..."):
                ...
                if generate_pdf:
                    generated_files['pdf'] = ReportGenerator.generate_pdf_report(
                ...
                if generate_txt:
                    generated_files['txt'] = ReportGenerator.generate_text_report(
                        st.session_state.analyzer.analysis_results,
                        os.path.join(output_dir, f"{base_filename}.txt"),
                        document_title="Multimodal Document Analysis Report"
                    )
                if generate_html:
                    generated_files['html'] = ReportGenerator.generate_html_report(
                        st.session_state.analyzer.analysis_results,
                        os.path.join(output_dir, f"{base_filename}.html"),
                        document_title="Multimodal Document Analysis Report"
                    )
                if generate_json:
                    generated_files['json'] = ReportGenerator.generate_json_report(
                        st.session_state.analyzer.analysis_results,
                ...
                for format_type, filepath in generated_files.items():
                    if filepath and os.path.exists(filepath):
                        ...
                        st.warning(f"Could not generate {format_type.upper()} report.")
        except Exception as e:
            st.error(f"Error generating reports: {str(e)}")


def show_settings():
    """Display settings page with error handling."""
    st.header("⚙️ Settings")

    st.subheader("User Profile")
    if st.session_state.current_user_id and st.session_state.db:
        try:
            ...
            st.error(f"Could not load user profile: {str(e)}")
...
def main():
    ...
    try:
        ...
        if hasattr(st.session_state, 'init_error'):
            st.warning(f"⚠️ {st.session_state.init_error}")
        if hasattr(st.session_state, 'db_error'):
            st.warning(f"⚠️ {st.session_state.db_error}")
